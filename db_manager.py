...
def get_user_by_name(userName):
    ...
    cursor = user_col.find(query).limit(50)
    arr = []
    if cursor:
        for item in cursor:
            response = {
...
def update_file_after_execute(fileId, origin, summary):
    query = {'_id': fileId}
    new_data = {'$set': {'recognizeText': origin,
                         'summaryText': summary, 'state': True}}
...
def get_list_id_room(idUser):
    ...
    setFollowerId = {}
    try:
        idFiles = file_col.find(
            {'followers': {'$elemMatch': {'_id': idUser}}}, {'idUser': 1})
        id_list = [doc['idUser'] for doc in idFiles]
        if id_list is not None:
            setFileId = set(id_list)
    except:
        pass
    ...
    try:
        ...
        if user is not None:
            idFollowers = user['follow']
        if idFollowers is not None and id_list is not None:
            setFollowerId = set(idFollowers)
    except:
        pass
...
def get_notify_by_id(idUser):
    try:
        cursor = notify_col.find(
            {'userReceive': {'$elemMatch': {'$in': [idUser]}}})
        if cursor is not None:
...
def insert_or_delete_like(evaluationEntity):
    ...
    if evaluationEntity['type'] == 'FILE':
        evaluation_query = {'_id': evaluationEntity['idFile'], "likes": {
            "$elemMatch": {"idUser": evaluationEntity['idUser']}}}
        ...
        try:
            ...
            if existing_evaluation:
                ...
                update = {
                    "$pull": {"likes": {"idUser": evaluationEntity['idUser']}}}
                result = file_col.update_one(query, update)
        ...
        try:
            ...
            if existing_evaluation is not None:
                ...
                update_action = {"$pull": {"comments.$.likes": {
                    "idUser": evaluationEntity['idUser']}}}
                ...
                update_action = {
                    "$push": {"comments.$.likes": evaluationEntity}}
                print("--------------- update query", update_action)
                result = file_col.update_one(query, update_action)
                return result.matched_count > 0
